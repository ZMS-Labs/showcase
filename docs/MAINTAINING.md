...
The browser check discovers every HTML page and exercises desktop and two narrow widths. It checks images, local links, horizontal overflow, existing controls, keyboard walkthroughs, focus, no-JavaScript access and transcripts. It briefly serves the static files on loopback to check all four videos, seeking, playback and descriptive tracks, then stops the server. Because that temporary Python server lacks byte-range support, its media check fully buffers the local recording before seeking; the deployed check exercises the host's actual seek behavior with metadata preload. Use `--base-url https://zms-labs.github.io/showcase/` to repeat the checks against the published copy. Optional `--screenshots <directory>` saves inspection images outside `docs/`. These checks cover the presentation; they do not certify the featured products or full accessibility conformance.

Several verifier expectations are deliberate, not duplication. The dual `EXPORT-MANIFEST.json` and `SHA256SUMS.txt` entry points are intentional: one structured manifest describes the payload and its scope, and one plain checksum list serves simple tooling; `site_manifest.py` writes both from the same walk. Each recording's duration and width are read from `docs/assets/manifest.json`, keyed by the asset's directory. The verifier requires the four expected recording directories, rejects missing or duplicate entries, and reports the number of completed playback checks. The literals that remain in `verify_site.py` — the descriptive-track cue counts and the recorded-check finding counts — are deliberate content gates; they change only with reviewed tracks or reviewed findings, never silently with a remux. New pages that use JavaScript must keep the noscript pattern the check enforces: with scripts disabled, the fallback message and the evidence links stay visible, as on the recorded-check page.

The genuine Steno, Krewcible, SaveBench and Neuraxic recordings have native controls, no autoplay, descriptive text tracks, selectable transcripts and synthetic content. Preserve their distinction from live product operation. Record new media provenance and inspect both pixels and metadata before replacing them. The Fleet Orchestrator and Steno Hallmark galleries use current component artwork with synthetic inputs; keep them distinct from archived prototypes, live connections, execution tests and real assessments. The verifier covers every gallery in addition to the original named product assertions.
...
The SaveBench and Neuraxic interface studies preserve existing product components and use synthetic state. Keep their recordings separate from SaveBench’s historical game evaluation and Neuraxic’s backend tests. SaveBench omits game artwork at the capture boundary; do not describe that as a product terrain toggle. Neuraxic uses its original reduced-motion shelf and local font fallbacks, and its prose offers are deterministic placeholders. Neither navigation recording establishes a complete product or a persisted workflow. The adjacent `interface-study.json` files describe scope; the asset manifest identifies the exact captures.
